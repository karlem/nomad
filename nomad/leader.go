...
func (s *Server) generateClusterID() (string, error) {
	if !ServersMeetMinimumVersion(s.Members(), minClusterIDVersion, false) {
		...
		return "", errors.New("cluster ID cannot be created until all servers are above minimum version")
	}
	...
	s.logger.Info("established cluster id", "cluster_id", newMeta.ClusterID, "create_time", newMeta.CreateTime)
